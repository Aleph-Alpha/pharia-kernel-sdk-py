...
class ChatStreamResponse(ABC):
    ...
    def message(self) -> Message:
        ...
        and a normal chat request should be used instead.
        """
        if not self._content:
            self._consume_stream()
        return Message(role=Role(self.role), content=self._content)
    ...
    def _consume_stream(self) -> None:
        ...
        if self._finish_reason is None or self._usage is None:
            raise ValueError(
                "Invalid event stream: must containt both finish reason and usage"
            )
    ...
    def stream(self) -> Generator[MessageAppend, None, None]:
        ...
        while (event := self.next()) is not None:
            match event:
                case MessageBegin():
                    raise ValueError(
                        "Invalid event stream: must only contain one message begin"
                    )
                case MessageAppend():
