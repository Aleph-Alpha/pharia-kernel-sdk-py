...
from pharia_skill.llama3.assistant import AssistantReply, ToolRequest, from_raw_response
...
def test_message_from_raw_response():
    ...
    message = from_raw_response(response)
...
def test_message_from_response_with_tool_call():
    ...
    message = from_raw_response(response, tools=[CodeInterpreter])
...
def test_message_tool_call_without_python_tag():
    ...
    message = from_raw_response(response, tools=[GetGitHubReadme])
    assert message.content is None
...
def test_non_existing_tool_is_not_parsed():
    """Llama3.3 does not prefix json tool calls with the python tag."""
    response = """{"type": "function", "name": "get_github_readme", "parameters": {"repository": "pharia-kernel"}}"""
    message = from_raw_response(response, tools=[])
...
def test_render_assistant_message():
    message = AssistantReply(content="Hello, world!")
    assert (
        message.render()
...
def deserialize_different_assistant_messages():
    ...
    assert isinstance(messages[0], AssistantReply)
    assert messages[0].tool_calls is None

    assert isinstance(messages[1], ToolRequest)
